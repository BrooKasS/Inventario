...
import type { BackupRun, BackupTipo } from "../../api/backups";
import {
  MAIN_GRADIENT,
  isAdmin,
  toIsoStart,
...
export default function SimpleBackupReportPage({ tipo, titulo, icono, descripcion }: Props) {
  ...
  async function cargar() {
    setLoading(true);
    try {
      const data = await listRuns(tipo);
      setRuns(data);
  ...
  const handleCrear = async () => {
    ...
    try {
      ...
      const run = await createRun(tipo, params);
  ...
  const handleDescargar = async (run: BackupRun) => {
    try {
      await downloadRunExcel(tipo, run.runId);
    } catch {
  ...
  const handleEnviar = async () => {
    ...
    setSendMsg(null);
    try {
      const updated = await sendRun(tipo, sendTarget.runId, {
        to: to ? parseEmailList(to) : undefined,
    ...
    >
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <p style={{ fontSize: 15, color: "rgba(255,255,255,.85)", margin: "6px 0 0" }}>{descripcion}</p>
        </div>
        ...
        >
          <div style={{ background: MAIN_GRADIENT, padding: "14px 20px" }}>
            <span style={{ fontSize: 14, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "#fff" }}>
              Nueva ejecución
            </span>
          </div>
          <div style={{ background: "#fff", padding: 18 }}>
            <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
              ...
              <PresetBtn label="Limpiar (usar rango por defecto)" onClick={() => { setDesde(""); setHasta(""); }} />
            </div>
            ...
            <div style={{ fontSize: 11, color: "#888", marginTop: 8 }}>
              Si no seleccionas un rango, se usa el rango por defecto de este tipo de informe.
            </div>
          </div>
        </div>

        {/* ── STAT CARDS (última ejecución seleccionada) ── */}
        {s && (
          <div
            style={{
              display: "grid",
          ...
          >
            ...
            <StatCard label="Completados" value={s.completed} grad="linear-gradient(135deg, #16A34A, #15803D)" />
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 760 }}>
                  <thead>
                    <tr style={{ background: "rgb(90, 56, 112)" }}>
                      {["Creado", "Rango", "Origen", "Correos", "Archivos", "% OK", "Enviado", "Acciones"].map((h) => (
                      ...
                      >
                        ...
                        <td style={tdStyle} onClick={(e) => e.stopPropagation()}>
                          <div style={{ display: "flex", gap: 6 }}>
                            ...
                            {admin && (
                              <IconBtn title="Eliminar" onClick={() => handleEliminar(r)}>🗑️</IconBtn>
                            )}
                          </div>
                        </td>
                      </tr>
        ...
        <div style={overlayStyle} onClick={() => !sending && setSendTarget(null)}>
          <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
            ...
            <p style={{ margin: "0 0 16px", fontSize: 12, color: "#777" }}>
              Ejecución del {formatFechaHora(sendTarget.creadoEn)}. Deja los campos vacíos para usar los destinatarios configurados por defecto.
            </p>
            ...
            <Field label="Asunto (opcional)">
              <input value={asunto} onChange={(e) => setAsunto(e.target.value)} style={inputStyle} placeholder={`Informe General ${titulo}`} />
            </Field>
...
function primaryBtnStyle(disabled: boolean): React.CSSProperties {
  return {
    ...
    borderRadius: 8,
    border: "none",
    background: disabled ? "#ddb8ae" : MAIN_GRADIENT,
    color: "#fff",
    fontWeight: 700,

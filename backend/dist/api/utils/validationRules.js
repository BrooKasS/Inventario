...
exports.isCreatedToday = isCreatedToday;
 ...
 */
function isCreatedToday(createdDate) {
 ...
 */
function validateCedula(val) {
    if (!val)
        return null;
    const str = String(val).trim();
    if (str.length > 10)
        return "Cédula: máximo 10 dígitos";
    if (!/^[\d\-]*$/.test(str))
        return "Cédula: solo números y guiones";
    return null;
}
/**
...
function validateIMEI(val) {
    ...
    const str = String(val).trim();
    if (str.length > 10)
        return "IMEI: máximo 10 dígitos";
    if (!/^\d+$/.test(str))
        return "IMEI deben ser solo números";
    return null;
}
/**
 * Valida número de línea: máx 10 dígitos
 */
function validateNumeroLinea(val) {
    if (!val)
        return null;
    const str = String(val).trim();
    if (str.length > 10)
        return "Número de línea: máximo 10 dígitos";
    if (!/^\d+$/.test(str))
        return "Número de línea: solo números";
    return null;
}
/**
 * Valida número de caso: máx 10 caracteres
 */
function validateNumeroCaso(val) {
    if (!val)
        return null;
    const str = String(val).trim();
    if (str.length > 10)
        return "Número de caso: máximo 10 caracteres";
    return null;
}
/**
 * Valida backup/monitoreo: SI/NO (case-insensitive)
 */
function validateBackupMonitoreo(val) {
    if (!val)
        return null;
...
function validateIP(val) {
    ...
    if (!match)
        return "IP: formato inválido (debe ser XXX.XXX.XXX.XXX)";
 ...
 */
function validateFecha(val, fieldName) {
    if (!val)
        return null;
 ...
 */
function validateAssetData(tipo, data, isNewRecord) {
    const errors = [];

 ...
 */
async function vincularVpnRegla(vpnId, conexion) {
    if (!conexion)
        return; // Sin IP, no hay vinculación
    // Buscar todas las VPNs con la misma conexión
    const vpnsConMismaConexion = await vpnRepo.find({
        where: { conexion },
        relations: ["asset"],
    });
    if (vpnsConMismaConexion.length < 2)
        return; // Solo una VPN, es principal
    ...
    if (!principal)
        return; // No se encontró principal clara
